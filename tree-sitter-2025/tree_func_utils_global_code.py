...
from tree_enums import GB_Code, ClassKeys
from tree_func_utils import query_method_node_called_methods, create_method_result
...
def get_global_code_info(language, root_node,gb_methods_ranges,gb_classes_ranges) -> Dict:
    ...
    # 返回结果字典
    gb_code_start_line = non_function_non_class_code[0][GB_Code.LINE.value] if non_function_non_class_code else None
    gb_code_end_line = non_function_non_class_code[-1][GB_Code.LINE.value] if non_function_non_class_code else None
    global_code_info = {
        GB_Code.START.value: gb_code_start_line,
        GB_Code.END.value: gb_code_end_line,
...
def get_global_code_str(global_code_info):
    if not global_code_info:
        return None

    # 提取 START 和 END 行号
    nf_start_line = global_code_info[GB_Code.START.value]
    nf_end_line = global_code_info[GB_Code.END.value]

    # 提取 BLOCKS 并按 LINE 排序
    code_blocks = global_code_info[GB_Code.BLOCKS.value]
    sorted_blocks = sorted(code_blocks, key=lambda x: x[GB_Code.LINE.value])

    # 构建完整的行号空代码
    codes = ["" for _ in range(nf_start_line, nf_end_line + 1)]
    # 填充代码数据
    for block in sorted_blocks:
        line_num = block[GB_Code.LINE.value]
        codes[line_num] = block[GB_Code.CODE.value]

    # 将所有代码拼接成字符串
    return "\n".join(codes)

def query_global_code_called_methods(parser, language, root_node, gb_classes_names, gb_classes_ranges, gb_methods_names, gb_methods_ranges, gb_object_class_infos):
    ...
    # 解析全局代码数据
    nf_global_code = get_global_code_str(global_code_info)
