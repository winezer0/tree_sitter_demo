...
def analyze_direct_method_infos(tree, language):
    ...
    # 获取文件中的所有函数信息
    methods_info = query_general_methods_info(tree, language, file_methods_names, classes_ranges)

    # 处理文件级别的函数调用
    if has_not_function_content(tree, classes_ranges, file_methods_ranges):
        non_function_info = query_not_method_called_methods(tree, language, classes_ranges, file_methods_ranges, file_methods_names)
