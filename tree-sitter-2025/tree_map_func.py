...
def build_method_map(parsed_infos:dict):
    ...
    for file_path, parsed_info in parsed_infos.items():
        ...
        for method_info in all_method_infos:
            method_info[MethodKeys.METHOD_FILE.value] = format_path(file_path)   # 填充文件路径信息
            method_info[MethodKeys.CALLED_BY_METHODS.value] = []                 # CALLED_BY_METHODS 预填充被调用的未知
...
def rev_find_local_method(called_method, method_name_info_map):
    ...
    if len(possible_methods) > 1:
        ...
        # 通过文件名进行筛选
        possible_methods =  [x.get(MethodKeys.METHOD_FILE.value) == called_method[MethodKeys.METHOD_FILE.value] for x in possible_methods]
        # 通过函数参数数量进行筛选
        possible_methods =  [len(x.get(MethodKeys.PARAMETERS.value)) >= len(called_method[MethodKeys.PARAMETERS.value]) for x in possible_methods]
...
def repair_called_methods(method_name_info_map):
    ...
    for method_full_name, method_infos in method_name_info_map.items():
        for method_info in method_infos:
            method_file = method_info.get(MethodKeys.METHOD_FILE.value)
            for called_method in method_info.get(MethodKeys.CALLED_METHODS.value):
                if called_method.get(MethodKeys.METHOD_TYPE.value) == MethodType.LOCAL_METHOD.value:
                    # 如果是本地方法 说明是被当前文件调用的,更新 METHOD_FILE 为 method_file
                    called_method[MethodKeys.METHOD_FILE.value] = method_file
                    # TODO 需要从建立的所有函数映射表中反查获取方法数据 并更新
                    find_infos = rev_find_local_method(method_name_info_map, called_method)

                elif called_method.get(MethodKeys.METHOD_TYPE.value) in [MethodType.BUILTIN_METHOD.value]:  #
                    # 如果是内置方法 说明数据保存了 内置方法函数,应该忽略掉
                    called_method[MethodKeys.METHOD_FILE.value] = method_file
                    # TODO 需要获取方法数据 并更新
                elif called_method.get(MethodKeys.METHOD_TYPE.value) in [MethodType.CUSTOM_METHOD.value, MethodType.DYNAMIC_METHOD.value]:
                    # 如果是自定义方法 还需要从别的文件进行查找 动态方法还不一定能查到
                    called_method[MethodKeys.METHOD_FILE.value] = "Need Find File"
                    # TODO 需要获取方法数据 并更新
                elif called_method.get(MethodKeys.METHOD_TYPE.value) in [MethodType.CLASS_METHOD.value, MethodType.CONSTRUCTOR.value]:
                    called_method[MethodKeys.METHOD_FILE.value] = "Need Find Class"
                else:
