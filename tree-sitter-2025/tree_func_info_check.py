...
from tree_sitter_uitls import extract_node_text_infos, find_children_by_field
...
def query_general_methods_define_infos(tree, language) -> Tuple[set, set[Tuple[int, int]]]:
    ...
    function_define_infos = extract_node_text_infos(tree.root_node, function_query, 'function.def', need_node_field='name')
...
def query_classes_define_infos(tree, language) -> Tuple[set[str], set[Tuple[int, int]]]:
    ...
    class_define_infos = extract_node_text_infos(tree.root_node, class_def_query, 'class.def', need_node_field='name')
...
def query_created_class_object_info(tree: object, language: object) -> list[dict]:
    ...
    object_class_dicts = []
    # 遍历匹配结果
    for match in new_object_query.matches(tree.root_node):
        match_dict = match[1]
...
def has_not_function_content(tree, class_ranges, function_ranges):
    """检查是否有非class和非函数的内容"""
    root_start = tree.root_node.start_point[0]
...
def guess_object_is_native(object_name, object_line, classes_names, object_class_infos):
...
def query_not_method_called_methods(tree, language, classes_names, classes_ranges, gb_methods_names, gb_methods_ranges,
    ...
    if nf_called_infos:
        ...
        nf_start_line = tree.root_node.start_point[0]
        nf_end_line = tree.root_node.end_point[0]
...
def res_called_construct_method(class_node, args_node, f_is_native):
    ...
    f_start_line = class_node.start_point[0]
    f_end_line = class_node.end_point[0]
...
def res_called_object_method(object_node, method_node, args_node, f_name_txt, f_is_native, f_is_static, f_called_class):
    """处理对象方法调用的通用函数
    Args:
    ...
    f_start_line = method_node.start_point[0]
    f_end_line = method_node.end_point[0]
